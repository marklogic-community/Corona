function jsonFromServer(json, success, error) {
  asyncTest(json, function() {
    $.ajax( { url: '/test/xq/isomorphic.xqy'
    , data: 'json=' + json
    , method: 'POST'
    , success: success
    , error: error
...
function badFromServerTest(json, message) {
  jsonFromServer( json,
    function(d,t,j) { equals(d, message, 'NOK') },
    function(j,t,e) { ok(true, 'OK') } ) }

function jsonFromServerTest(json) {
  jsonFromServer( json,
    function(d,t,j) { 
      equals(d, json, 'OK') },
    function(j,t,e) { ok(false, 'NOK') } ) }

function queryParser(json, path, success, error) {
  asyncTest(json, function() {
    $.ajax( { url: '/test/xq/parse.xqy'
    , data: 'json=' + json + '&path=' + path
    , method: 'POST'
    , success: success
    , error: error
    , complete: function() { start(); } }) }) }

function queryParserTest(json, path) {
  queryParser( json, path,
...
badJSON = [
    {
        "json": "[1 2]",
        "error": "Logged Exception: Expecting end tag"
    },
    {
        "json": "{\"foo\" \"bar\"}",
        "error": "Logged Exception: Expecting end tag"
...
$(document).ready(function(){
  module("Bad JSON") ;
    ...
    for (var i = badJSON.length - 1; i >= 0; i--) {
      badFromServerTest(badJSON[i].json, badJSON[i].error) }
  module("Good JSON") ;
    for (var i = validJSON.length - 1; i >= 0; i--) {
      jsonFromServerTest(JSON.stringify(validJSON[i].json)) }
  module("Query Parser") ;
    // Missing Invalid JSON
    for (var i = validJSON.length - 1; i >= 0; i--) {
      if (validJSON[i].path) { 
        queryParserTest(JSON.stringify(validJSON[i].json), validJSON[i].path) } }
  // Missing REST
