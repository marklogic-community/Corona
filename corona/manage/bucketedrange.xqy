    ...
    then
        ...
        then
            ...
            then
                ...
                if($mode = "json")
                then manage:createJSONAutoBucketedRange($name, $key, $type, map:get($params, "collation"), $bucketInterval, $startingAt, $stoppingAt, $firstFormat, $format, $lastFormat, $config)
                else if($mode = "xmlattribute")
                then manage:createXMLAttributeAutoBucketedRange($name, $element, $attribute, $type, map:get($params, "collation"), $bucketInterval, $startingAt, $stoppingAt, $firstFormat, $format, $lastFormat, $config)
                else if($mode = "xmlelement")
                then manage:createXMLElementAutoBucketedRange($name, $element, $type, map:get($params, "collation"), $bucketInterval, $startingAt, $stoppingAt, $firstFormat, $format, $lastFormat, $config)
                else ()
            else if(exists($buckets))
            then
                if($mode = "json")
                then manage:createJSONBucketedRange($name, $key, $type, map:get($params, "collation"), $buckets, $config)
                else if($mode = "xmlattribute")
                then manage:createXMLAttributeBucketedRange($name, $element, $attribute, $type, map:get($params, "collation"), $buckets, $config)
                else if($mode = "xmlelement")
                then manage:createXMLElementBucketedRange($name, $element, $type, map:get($params, "collation"), $buckets, $config)
                else ()
