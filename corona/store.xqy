    ...
    then xdmp:invoke("/corona/store.xqy", (), <options xmlns="xdmp:eval"><transaction-id>{ map:get(common:processTXID($txid, true()), "id") }</transaction-id></options>)
        ...
        then try {
            ...
            return
                ...
                else store:insertDocument($uri, $bodyContent, $collections, $properties, $permissions, $quality, $contentType, map:get($params, "language"), map:get($params, "repair"), $applyTransform, $params, $respondWithContent)
        }
        catch ($e) {
            common:errorFromException($e, $outputFormat)
        }
            ...
            return
                ...
                then store:moveDocument($uri, map:get($params, "moveTo"))
                    ...
                    then
                        ...
                        else store:updateDocumentContent($uri, $bodyContent, $contentType, map:get($params, "language"), map:get($params, "repair"), $applyTransform, $params, $respondWithContent)
                    else (),
