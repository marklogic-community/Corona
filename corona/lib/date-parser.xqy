...
) as xs:string
{
    let $groups :=
        for $token in $format/*
        return
            if(local-name($token) = "whitespace")
            then "\s+"
...
) as xs:string?
{
	...
	then
		...
		then
			...
			then "1900"
			else concat("20", dateparser:stringPad("0", 2 - string-length($year)), $year)
		else if(xs:integer($year) >= 100 and xs:integer($year) < 200)  (: 102 = 2002, thanks Java :)
...
) as xs:string
{
	...
	return
		if(exists($parsedMonth))
		then $parsedMonth
		else dateparser:expandTwoDigits($month, "01")
};
...
) as xs:string
{
    dateparser:expandTwoDigits(
        if(matches($day, "^(\d?\d)(st|nd|rd|th)$"))
        then replace($day, "st|nd|rd|th", "")
        else $day
    , "01")
};
...
) as xs:string
{
	...
	then $default
	else concat(dateparser:stringPad("0", 2 - string-length($num)), $num)
};
